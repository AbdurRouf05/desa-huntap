...
import { perangkatDesa } from "@/lib/dummy-data";
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="max-w-md mx-auto mb-12">
            <div className="bg-white rounded-2xl border-2 border-primary/20 p-8 text-center shadow-lg hover:shadow-xl transition-shadow">
              <div className="w-28 h-28 rounded-full bg-gradient-to-br from-primary/20 to-emerald-100 mx-auto mb-4 flex items-center justify-center border-4 border-white shadow-lg">
                <UserCheck className="w-12 h-12 text-primary" />
              </div>
              <h3 className="text-xl font-black text-slate-800">{perangkatDesa[0].name}</h3>
              <span className="inline-block mt-2 px-4 py-1.5 bg-primary/10 text-primary text-sm font-bold rounded-full">
                {perangkatDesa[0].position}
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
            {perangkatDesa.slice(1).map((person) => (
              ...
              >
                <div className="w-20 h-20 rounded-full bg-gradient-to-br from-slate-100 to-slate-50 mx-auto mb-4 flex items-center justify-center border-2 border-slate-200 group-hover:border-primary/30 transition-colors">
