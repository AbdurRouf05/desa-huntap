...
export default async function KategoriProdukAdminPage() {
    const categories = await productCategoryService.getList();

    return (
        <main className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-black text-slate-900 tracking-tight">Kategori Produk UMKM</h1>
                    <p className="text-slate-500 text-sm">Kelola kategori produk untuk toko dan status kemunculannya di portal</p>
                </div>
                <Link 
                    href="/cp/kategori-produk/tambah"
                                ...
                                <tr key={item.id} className="transition-all group hover:bg-slate-50">
                                    ...
                                    <td className="px-6 py-5 text-right">
                                        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
                                            <Link
                                                href={`/cp/kategori-produk/edit/${item.id}`}
